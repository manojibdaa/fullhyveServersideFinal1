...
import { testUser, testTeam, testProject, testSet, testTask, testAnnouncement, testProjectUser, testProjectTeams, testMessages, testTeamMembers, testContacts, testContactsReq, testTeamMembersReq } from "./SeedData";
...
export class TestDb{
    ...
    public static initDb(newDb:boolean){
        ...
        .then(async ()=>{
            await DB.Team.bulkCreate(testTeam)
            // return new Promise((resolve, reject)=>{
        ...
        .then(async ()=>{
            await DB.Contact.bulkCreate(testContactsReq);
        })
        
        .then(async ()=>{
            await DB.TeamUser.bulkCreate(testTeamMembersReq);
        })
        ...
        .then(async ()=>{
            await DB.Message.bulkCreate(testMessages);
        })
    ...
    static addMessages(){
        return DB.Message.bulkCreate(testMessages);
    }
