...
export class ProjectS{
    ...
    static getTaskSets(userId:number, projectId:any, offset:number=0,limit:number=ProjectConst.PROJECTS_SEARCH_LIMIT){
        return new Promise((resolve, reject)=>{
            ...
            .then((project:any)=>{
                ...
                if(project){
                    ...
                    tasksetsReturn.tasksets = UtilMethods.getTasksetAttr(userId,project.tasksets);
                }
    ...
    static newProject(projectData:any){
        return new Promise((resolve, reject)=>{
            let rProjectId:any;
            ProjectDb.newProject(projectData)

            .then((project:any):any=>{
                if(project){
    ...
    //********************************************************** */
    static newSet(userId:number, setData:any){
        return ProjectDb.newSet(setData)

        .then((taskset:any)=>{
            taskset['tasks'] = [];
            return UtilMethods.getTasksetAttr(userId, [taskset])[0];
        })
    }
    ...
    static checkContributor(userId:number, projectId:number):Promise<boolean>{
        return new Promise((resolve, reject)=>{
            ...
            .then((status:any)=>{
                ...
                return ProjectDb.checkIndividualContributor(userId, projectId);
            })
    ...
    static checkTeamContributorRequestReceiever(userId:number, requestId:number):Promise<boolean>{
        return new Promise((resolve, reject)=>{
            ProjectDb.checkTeamContributorRequestReceiver(requestId)

            .then((request:any)=>{
                if(request){
                    return TeamDb.checkTeamLeadership(userId, request.teamId);
                }
                resolve(false);
            })
