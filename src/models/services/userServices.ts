...
export class UserS{
    static isUserNameAvailable(userName:string, callback:any):Promise<boolean>{
        return new Promise((resolve, reject)=>{
            UserDb.getByUserName(userName)

            .then((user:any)=>{
                if(user){
                    callback({success:false, code:400, message:"Username is already used",data: null});
                }else{
                    resolve(true);
                }
            })
    ...
    static unfriend(requestId:any){
        return UserDb.removeFriend(requestId);
    }
