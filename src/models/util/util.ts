import {User,MyTeam,Team,ReturnAttrType,FastSearchResult,Project,MinTeam,MinUser,NotificationType, Task, Identity, TaskStatus, SearchFor, RequestStatus} from '../models/serviceValues';
import {AnnouncementConst, UserConst, TeamConst} from '../constants/constant';
...
export class UtilMethods{
    ...
    static getTeamAttr(teams:any, type:SearchFor=SearchFor.General){        
    ...
    static getContributorCount(project:any, leaderId:number){
    ...
    static getProjectAttr(projects:any):Project[]{
        ...
        for(let project of projects){
            ...
            let leader:User = UtilMethods.getUserAttr([project.user])[0];
            let contributors = UtilMethods.getContributorCount(project, Number.parseInt(leader.id));
        ...
        for(let a of announcements){
            ...
            if(showReply){
                ...
                for(let b of replies){
                    let rSender = UtilMethods.getUserAttr([b.user]);
                    annR.replies.push({
                        id:b.id,
                        message:b.message,
    ...
    static getTasksetAttr(userId:number, sets:any){
        let setsReturn:any = [];
        ...
        for(let set of sets){
            ...
            for(let task of set.tasks){
                ...
                if(task.status!=null && task.status == TaskStatus.Approved){
                    completion++;
                }
            }
    ...
    static getRequestStatus(userId:number, senderId:number, request:string):RequestStatus{
        switch(request){
            case UserConst.REQUEST.ACCEPTED: {
                return RequestStatus.ACCEPTED
            }

            case UserConst.REQUEST.UNDECIDED:{
                if(userId==senderId){
            ...
            case UserConst.REQUEST.REJECTED:
            case UserConst.REQUEST.REMOVED:
            {
